...
public class ProblemEleven {
	...
	//	findNumberInArray(4);
		...
		for (int j=0;j<=19;j++) {
			for (int i=0;i<=19;i++){
				numberArray[j][i]=findNumberInArray(counter);
				counter++;
			}
		}
		counter=0;
		...
		for (int j=0;j<=19;j++) {
			for (int i=0;i<=19;i++){
				largestProduct = isLargest(checksMuliples(numberArray, j, i),largestProduct);
				counter++;
			}
		}
	...
	private static int checksMuliples(int[][]  numberArray, int rowNumber, int columnNumber){
		int largestProduct = 0;
		int sum = 0;
	...
	private static int isLargest(int sum, int largestProduct){
	...
	private static int findNumberInArray(int nextNumber){
		int returnNumber;
